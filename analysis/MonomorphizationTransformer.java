...
/**
 * MonomorphizationTransformer — SceneTransformer running in wjtp.
 *
 * Three refinement layers:
 *   L1  CHA   — read Soot's call graph (upper bound, built by Soot already)
 *   L2  PTA   — intra-procedural points-to (your assignment engine)
 *   L3  k-Obj — use caller's PTA field state to narrow cross-method sites
 *
 * Classification:
 *   MONO      (1 target)  → devirtualise + inline if small
 *   BIMORPHIC (2 targets) → guarded inline
 *   POLY      (3–4)       → type-test chain
 *   MEGA      (5+)        → leave as virtual
 */
public class MonomorphizationTransformer extends SceneTransformer {
    ...
    protected void internalTransform(String phaseName, Map<String, String> options) {
        ...
        printDistribution("After CHA", sites, false, false);

        refinePTA(sites);
        printDistribution("After PTA", sites, true, false);
